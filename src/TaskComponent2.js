...
const TaskComponent2 = () => {
    ...
    useEffect(() => {
        const fetchCompletedTasks = async () => {
            try {
                const response = await axios.get('https://task.pooldegens.meme/api/get_completed_tasks2', {
                    params: { username: username }
                });
                const completedTaskIds = response.data.completedTasks2 || [];
    ...
    const handleTaskClick = (task) => {
        ...
        setTimeout(async () => {
            ...
            updatePlayButtonState(); // Update the play button state after marking the task as completed
        }, 7000);
    };
    ...
    const saveTaskProgress = async (taskId, completed) => {
        try {
            const response = await fetch('https://task.pooldegens.meme/api/complete_task2', {
                method: 'POST',
                headers: {
